...
try:
    ...
    check("pinned designs: marker pin round-trips with a verbatim config",
          ...
          f"(got {s_pm}/{s_pg})")
    # three near-cap .dat entries per pin: 40 x ~357 KB ~ 14.3 MB — over
    # the 12 MB store cap while staying under the 16 MB streamed body cap,
    # so the refusal exercised is the store's, not the body reader's
