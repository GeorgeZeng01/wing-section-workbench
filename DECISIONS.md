...
## The pinned-designs library (2026-08-03)

**Two-tier with auto-mirror, not a globalized pin store.** Workspace pins
keep their exact semantics — wiped on preset switch, replaced on project
open, saved in sessions and project files, feeding that project's
"Design iterations" report section; the written rationale for the wipe
(pins belong to their design context) stands. Every pin is now ALSO
auto-mirrored into a durable machine-level library
(app_data/pinned_designs.json) that survives all of it, rendered by its
own Pinned tab with an N-way compare. Globalizing the workspace store was
rejected: it breaks the report's per-project record, makes every
pin/delete network-gated, and orphans pins in project files opened on
another machine. The mirror is fire-and-forget — a library failure toasts
and the workspace pin stands.

**The store copies the preset-library discipline, generalized.** The
_presets_read/_presets_write/_presets_stale helpers took a key= kwarg
(default preserves the existing libraries byte-identically) so pins are
not stored under a misnamed field forever; same optimistic rev token,
...
the Load path re-registers and remaps specs exactly as project-open does
— a config referencing an unregistered custom: spec must PASS store
